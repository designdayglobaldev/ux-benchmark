...
const MOCK_PATTERN = {
  ...
  status: 'LIVE',
  screens: [
    { id: 'scr-1', appName: 'Revolut', screenName: 'Advanced Settings' },
    { id: 'scr-5', appName: 'Monzo', screenName: 'Overdraft Application' },
  ]
}
      ...
      <Header>
        <div className='flex items-center gap-4'>
          ...
          <div className='flex items-center gap-2'>
            <h1 className='text-lg font-semibold'>{pattern.title}</h1>
            <Badge variant={pattern.status === 'LIVE' ? 'default' : 'secondary'}>{pattern.status}</Badge>
          </div>
        </div>
        <div className='ml-auto flex items-center gap-4'>
          <Button variant='outline' asChild>
            <Link to='/patterns/$patternId/edit' params={{ patternId: pattern.id }}>
              <Edit className='mr-2 h-4 w-4' /> Edit Pattern
            </Link>
          </Button>
          <ProfileDropdown />
        </div>
        ...
        <div className='flex items-center justify-between mb-4'>
          <div>
            ...
            <p className='text-muted-foreground'>
              Screens that currently exhibit this UX pattern.
            </p>
          </div>
        </div>
